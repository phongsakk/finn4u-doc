...
function page() {
  ...
  return (<>
    {loading ? <LoadPage /> : <div className="container mt-3 py-2 p-lg-4 bg-white">
      {asset?.images && <AssetPicture images={asset?.images as []} />}
      <label className=" my-4 h2">
        {asset?.title}
      </label>
      <Link
        href={String(asset?.google_map_location)}
        target="_blank"
        className="text-primary h4 font2 d-flex"
        style={{ textDecoration: "" }}
      >
        <IconCusttom src="/mdi_location.svg" /> <ins>{asset?.location}</ins>
      </Link>
      ...
      <div className="d-flex mb-4">
        ...
        <span className="font2">
          ...
          <span className="px-2">{String(asset?.id).padStart(5, "0")}</span>
        </span>
      </div>
      <div className="d-flex mb-4">
        <IconCusttom src="/ic-lo2.svg" />
        <span className="font2">{Number(asset?.square_meter)} ตารางวา</span>
      </div>
      <div className="d-flex mb-4">
        ...
        <div className="col-lg-3">
          <div className="border">
            <div className="info-header border-bottom p-3 d-flex flex-column">
              ...
              <div className="text-center">ลงประกาศโดย : {asset?.owner?.fullname} <FaCheckCircle className="text-success" /></div>
            </div>
            <div className="info-body p-3 d-flex flex-column gap-3">
              <div className="btn btn-light text-start px-3"><FaPhoneAlt size={20} />{asset?.owner?.phone_number}</div>
              <div className="btn btn-light text-start px-3"><FaLine size={20} />{asset?.owner?.line}</div>
              <div className="btn btn-light text-start px-3"><TbMailFilled size={20} />{asset?.owner?.email}</div>
            </div>
          </div>
        </div>
      </Row>

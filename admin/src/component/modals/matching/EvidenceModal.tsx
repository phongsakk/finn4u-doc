...
function EvidenceModal({
    ...
    const boot = async () => {
      try {
        ...
        const { data: res } = await axios.get(
          api.internal(`/api/matching/${id}/evidence`)
        );
        if (res?.status) {
          setForm(res?.data);
        }
  ...
  console.log(form)
  return (
    <Modal size="lg" show={prompt.show} onHide={Close} centered>
      <Modal.Body>
        ...
        <div className="container">
          {loading ? (
            ...
            <>
              <EvidenceResult
                name={DownLoadForm.lease_agreement}
                href={DownLoadForm.lease_agreement_href}
                evidencePath={form?.lease_agreement}
              />
              <EvidenceResult
                name={DownLoadForm.spouse_consent_letter}
                href={DownLoadForm.spouse_consent_letter_href}
                evidencePath={form?.spouse_consent_letter}
              />
              <EvidenceResult
                name={DownLoadForm.real_estate_agent_appointment_agreement}
                href={DownLoadForm.real_estate_agent_appointment_agreement_href}
                evidencePath={form?.real_estate_agent_appointment_agreement}
              />
              <EvidenceResult
                name={DownLoadForm.purchase_and_sale_agreement}
                href={DownLoadForm.purchase_and_sale_agreement_href}
                evidencePath={form?.purchase_and_sale_agreement}
              />
              <EvidenceResult
                name={DownLoadForm.property_detail_sheet}
                href={DownLoadForm.property_detail_sheet_href}
                evidencePath={form?.property_detail_sheet}
              />
              <EvidenceResult
                name={DownLoadForm.power_of_attorney}
                href={DownLoadForm.power_of_attorney_href}
                evidencePath={form?.power_of_attorney}
              />
              <EvidenceResult
                name={DownLoadForm.condo_power_of_attorney}
                href={DownLoadForm.condo_power_of_attorney_href}
                evidencePath={form?.condo_power_of_attorney}
              />
            </>
          )}
        </div>
...
const EvidenceResult = ({
  ...
  return (
    <Row className="align-items-center mb-3">
      ...
      <div className="col-3">
        {evidencePath ? (
          <Link href={`/api/uploads/${evidencePath}`} target="_blank" className="btn btn-outline-primary">
            ดูเอกสาร
          </Link>
        ) : (
          <span className="text-muted">ไม่มีเอกสาร</span>
        )}{" "}
      </div>
    </Row>
  );
